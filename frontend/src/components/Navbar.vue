<template>
  <div class="h-screen w-64 bg-navbar text-white flex flex-col">
    <div class="flex-grow p-4 overflow-auto">
      <button 
        @click="goBack" 
        class="w-full bg-warning hover:bg-danger text-white px-4 py-2 rounded transition mb-2">
        Wstecz
      </button>
      ...
      <router-link v-if="userRole === 'manager'" to="/systemconf" class="text-white block bg-secondary hover:bg-accent p-3 rounded mb-3 transition">Ustawienia systemu</router-link>
    </div>
